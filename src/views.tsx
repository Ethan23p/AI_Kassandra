...
export const LoginPage = (props: { error?: string }) => (
    <Layout title="Login">
        <form hx-post="/login" hx-target="body" class="flex flex-col gap-6 items-center">
            {props.error && <div class="text-red-400 bg-red-900/20 px-4 py-2 rounded">{props.error}</div>}
            <div class="flex flex-col w-full max-w-sm">
                <label class="text-sm text-gray-400 mb-2 uppercase tracking-wide">Passkey</label>
                <input
                    type="password"
                    name="password"
                    class="bg-gray-900 border border-gray-700 p-3 rounded text-center tracking-widest focus:outline-none focus:border-indigo-500 transition-colors"
                    placeholder="••••••••"
                    required
                />
            </div>
            <div class="flex flex-col w-full max-w-sm mt-4">
                <label class="text-sm text-gray-400 mb-2 uppercase tracking-wide">Identity</label>
                <input
                    type="text"
                    name="username"
                    class="bg-gray-900 border border-gray-700 p-3 rounded text-center focus:outline-none focus:border-indigo-500 transition-colors"
                    placeholder="Enter your name"
                    required
                />
            </div>
            <button type="submit" class="mt-4 px-8 py-3 bg-indigo-900 hover:bg-indigo-800 text-indigo-200 rounded transition duration-300 uppercase tracking-widest text-sm">
                Enter
            </button>
        </form>
    </Layout>
